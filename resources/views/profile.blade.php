            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    ...
                    <div class="flex flex-col items-center">
                        <div
                            class="w-32 h-32 rounded-full bg-[#280905] 
                                 flex items-center justify-center text-white text-4xl font-bold mb-4
                                 shadow-lg border-4 border-slate-100">
                            {{ strtoupper(substr(Auth::user()->name ?? 'A', 0, 1)) }}
                        </div>

                        <h4 class="text-xl font-bold text-[#280905]">{{ Auth::user()->name }}</h4>
                        <p class="text-sm text-[#280905]">{{ Auth::user()->email }}</p>

                        <div class="mt-4 w-full">
                            <div class="bg-slate-50 rounded-xl p-3 text-center border border-slate-100">
                                <span class="px-3 py-1.5 bg-[#280905] text-white rounded-full text-xs font-medium">
                                    <i class="fa-solid fa-shield-halved mr-1"></i> Siswa
                                </span>
                            </div>
                        </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    ...
                    <form action="{{ route('profile.update') }}" method="POST" class="space-y-6">
                        ...
                        <div class="flex gap-3">
                            <button type="submit"
                                class="px-6 py-3 bg-[#280905] text-white rounded-xl hover:bg-slate-800 
